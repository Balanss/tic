...
{board.map((board,i) => {   
    if(board.zero === "X"  && board.three === "X" && board.six === "X"){
      ...
      return (  <div key={i} className='windiv'> <h2 className='wintitle'>{board.host}  Wins </h2>  </div>)

    } else if (board.zero === "O" && board.three  === "O" && board.six === "O"){
      const docRef=doc(db,room,board.id);const payload = { Disabled : true}; updateDoc(docRef, payload);
      return (<div  key={i} className='windiv'> <h2 className='wintitle'>{board.friend} Wins </h2>   </div>)
    ...
    return (<div key={i} className='windiv'>  <h2 className='wintitle'> {board.host} Wins </h2>   </div>)
        ...
        const docRef=doc(db,room,board.id);const payload = { Disabled : true}; updateDoc(docRef, payload);
        return(<div key={i} className='windiv'> <h2 className='wintitle'> {board.host} Wins </h2>{board.host}   </div>)
